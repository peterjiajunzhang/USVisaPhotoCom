...
async function main() {
  ...
  const framePath = path.resolve(__dirname, "frame.html");
  ...
  for (const t of targetList) {
    ...
    for (const screen of cfg.screens) {
      ...
      // file:// pages can't reliably fetch local JSON; inject config before scripts run.
      await page.addInitScript((cfg) => {
        window.__SHOT__ = cfg;
      }, shotCfg);

      await page.goto(frameUrl, { waitUntil: "load" });
